...
def print_features(feature_groups, file_path: str):
    with gzip.open(file_path, 'wt') as handle:
        for gene_id in feature_groups:
            feature_group = feature_groups[gene_id]
            if feature_group.ncRNA is not None:
                continue
            handle.write('###\n')
            ...
            line = '\t'.join(
                [
                    feature_group.gene.chrom,
                    'EMBL',
                    'gene',
                    feature_group.gene.start,
                    feature_group.gene.stop,
                    '.',
                    feature_group.gene.strand,
                    '.',
                    gene_attributes,
                ]
            )
            ...
            line = '\t'.join[
                feature_group.transcript.chrom,
                'EMBL',
                'transcript',
                feature_group.transcript.start,
                feature_group.transcript.stop,
                '.',
                feature_group.transcript.strand,
                '.',
            ...
            for c in feature_group.CDSs:
                line = '\t'.join([c.chrom, 'EMBL', 'CDS', c.start, c.stop, '.', c.strand, '0', cds_attributes])
                print(line + '\n')
...
def main():
    ...
    assert len(sys.argv) >= 3, 'Usage: gff2gff.py <gff_inhandle> <path to new gff3> [optional path to sqlite db]'

    if len(sys.argv) == 4:
        sqlite_db = sys.argv[3]
    else:
        sqlite_db = 'local.sqlite'  # :memory: was SLOW
