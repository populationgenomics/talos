...
* Step 1: Read a MatrixTable generated by AnnotateCohort
    - Strip out all the prior annotation content
    - Subset the MT to samples in this Cohort
    - Write a directory of VCF fragments, each containing a separate header

* Step 2: Annotate with gnomAD frequencies and AlphaMissense data
    - Parallelised over each fragment from step 1
    - Annotate each fragment with gnomAD and AlphaMissense results in one pass

* Step 3: Annotate each VCF with per-transcript consequences
    - Uses bcftools csq, with a GFF3 file and reference genome

* Step 4: Ingest each VCF fragment as a MatrixTable
    - Loads up each annotated VCF into a Hail MatrixTable
    - Splits up the chunky BCSQ annotations into a Hail Array of Hail Structs
    - Adds in per-transcript annotation from MANE
"""
...
def get_manifest_fragments(vcf_dir: str) -> SortedDict:
    ...
    with manifest.open() as f:
        return SortedDict({f'part_{i}': dir_path / line.rstrip() for i, line in enumerate(f)})
...
@stage.stage(required_stages=[EncodeAlphamissense, ExtractVcfFromMt])
class AnnotateUsingEchtvar(stage.CohortStage):
    ...
    def queue_jobs(self, cohort: targets.Cohort, inputs: stage.StageInput) -> stage.StageOutput:
        ...
        fragment_dir = inputs.as_str(cohort, ExtractVcfFromMt, key='vcf_dir')
        fragments = get_manifest_fragments(fragment_dir)
...
@stage.stage(required_stages=[ExtractVcfFromMt, AnnotateUsingEchtvar])
...
@stage.stage(
    required_stages=[ExtractVcfFromMt, AnnotatedVcfIntoMt],
