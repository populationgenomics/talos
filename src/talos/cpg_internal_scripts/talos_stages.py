...
class AnnotateAndLabelMito(stage.CohortStage):
    ...
    def queue_jobs(self, cohort: Cohort, inputs: stage.StageInput) -> stage.StageOutput:
        ...
        label_job.command(
            ...
            gcloud storage cp --quiet -r {clinvarbitration_ht} "$BATCH_TMPDIR"
...
class RunHailFiltering(stage.CohortStage):
    ...
    def queue_jobs(self, cohort: targets.Cohort, inputs: stage.StageInput) -> stage.StageOutput:
        ...
        job.command(f'gcloud storage cp --quiet -r {clinvarbitration_hts["decisions"]} "$BATCH_TMPDIR"')
        job.command(f'gcloud storage cp --quiet -r {clinvarbitration_hts["pm5"]} "$BATCH_TMPDIR"')

        # read in the MT using gcloud, directly into batch tmp
        job.command(f'gcloud storage cp --quiet -r {input_mt!s} $BATCH_TMPDIR')
