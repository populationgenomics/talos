...
GREEN_TEMPLATE = 'https://panelapp-aus.org/api/v1/panels/{id}/genes/?confidence_level=3'
...
async def get_all_known_panels(panel_ids: set[int]) -> dict[int, dict]:
    """
    ...
    return {pid: data for panel in all_panel_details for pid, data in panel.items()}
...
def main(output: str):
    ...
    all_panels = set(panels_and_hpo_terms.keys())
