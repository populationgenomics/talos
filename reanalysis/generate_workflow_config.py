...
parser.add_argument("--driver_image", help="Hail driverimage name (relative to main deployment CR) to target")
parser.add_argument("--output_prefix", help="subpath on dataset's hail bucket to use for output", required=True)
...
if args.extra_configs:
    for extra_config in args.extra_configs:
        update_dict(config, toml.load(extra_config))
